...
int ConnectToEndpoints(zmq::socket_t* readrep_sock, std::map<std::string,Store*> &readrep_conns, zmq::socket_t* write_sock, std::map<std::string,Store*> &write_conns, zmq::socket_t* log_sock, std::map<std::string,Store*> &log_conns); ///< Add to standard ports assumed to be associated with all found services.
